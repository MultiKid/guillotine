...
export function createViewerPlayerView(player: Player): ViewerPlayerView {
  return {
...
export function createPlayerGameView(state: GameState, viewerPlayerId: PlayerId): PlayerGameView {
  ...
  return {
    ...
    notice: state.notice,
    players: state.players.map(createPublicPlayerView),
    viewer: viewerPlayer ? createViewerPlayerView(viewerPlayer) : undefined,
